...
.vscode/          Editor settings, so the project opens the same for everyone
```

Three directories exist on disk but are not part of the project and are
gitignored: `node_modules/` (installed packages), `.next/` (compiled output)
and `.husky/_/` (git-hook shims). All three are regenerated by `npm install`
and `npm run build`, none can be deleted, and `.vscode/settings.json` hides
them from the Explorer and from search.
